...
| Category membership is positional, not relational. | DB comment declares positional membership (`supabase/migrations/20260604211412_create_question_ownership_contract.sql:15`). | UI derives category by walking the current question base (`src/components/offers/OfferPreparationResult.astro:53`, `src/components/offers/OfferPreparationResult.astro:57`). | Open decision. Unknown whether positional membership will be upheld in the future. |
...
| Extraction should finish within 60 seconds. | PRD non-functional requirement (`context/foundation/prd.md:93`). | Timeout constant is 55 seconds (`src/lib/services/extraction-contract.ts:5`) and provider aborts using that timeout (`src/lib/services/extraction-provider.ts:53`, `src/lib/services/extraction-provider.ts:55`). | Timebox may change in the future. |

### Decisions Log

| ID | Decision | Planning impact |
|---|---|---|
| D-001 | Buyer offer content privacy is temporary MVP behavior; sharing or public offers/preparations may be explored later. | Do not design the aggregate name as permanently private-only. Keep ownership explicit so sharing can become a separate policy later. |
| D-002 | Saved offers are read-only only for this slice; future work may add more sources for the same offer or append offer content. | Do not bake immutability of offer content into the core aggregate. Treat current read-only behavior as a repository/API policy. |
| D-003 | Resetting the buyer question base to the active template document is temporary until template management is explored. | Keep reset out of the chosen core aggregate. Future template/version management may need its own aggregate or policy. |
| D-004 | Positional category membership is undecided for the long term. | Snapshot category/order at preparation time, but do not assume positional grouping is the permanent domain model. |
| D-005 | The 60-second extraction timebox may change. | Keep the timeout configurable and outside the aggregate's core invariant. The aggregate should reject invalid results, not own provider-duration policy. |
